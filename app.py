...
# --- 2. REFINED MCX ENGINE (Auto-Roll Based on Expiry) ---
@st.cache_data(ttl=600)
def get_mc_guinea_price():
    ...
    # Calculate Exact Expiry Date (Last Business Day of current month)
    last_day_num = calendar.monthrange(today.year, today.month)[1]
    expiry_date = datetime.date(today.year, today.month, last_day_num)
    
    # Adjust for weekends: MCX expires on Friday if month ends on Saturday (5) or Sunday (6)
    ...
    # Determine the contract code based on the 6-day rule
    if today > expiry_date:
        mcx_code = "MGG01" # Post-expiry, MGG01 is the new month
    else:
        days_to_expiry = (expiry_date - today).days
        mcx_code = "MGG02" if days_to_expiry <= 6 else "MGG01"

    url = f"https://priceapi.moneycontrol.com/pricefeed/mcx/futures/{mcx_code}"
    
    try:
        res = requests.get(url, headers=headers, timeout=5).json()
        # 'buyprice' is the Best Bid on the MCX feed
        best_bid = float(res['data'].get('buyprice', 0.0))
        ltp = float(res['data'].get('pricecurrent', 0.0))
        
        price = best_bid if best_bid > 0 else ltp
        source_label = f"Live {mcx_code} Bid"
